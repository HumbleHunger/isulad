...
oci_runtime_spec *default_spec(bool system_container)
{
    ...
    // 从/etc/default/isulad/config.json中的default_spec添加入oci_spec
                    ...
                    oci_runtime_spec *oci_spec)
{
    ...
    {
        host_spec->user_remap = conf_get_isulad_user_remap();
    }
    /*
    if (host_spec->user_remap != NULL && host_spec->system_container){
        user_remap = host_spec->user_remap;
    }
    else {
