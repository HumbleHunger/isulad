...
// 修改isulad engines lcr相关权限，使其它用户能够访问目录
...
static int do_start_container(container_t *cont, const char *console_fifos[], bool reset_rm, pid_ppid_info_t *pid_info)
{
    ...
    // user_remap前的准备工作，如改变engine/lcr目录的访问权限等.(考虑在这进行权限的更改)
    ...
    // 从/var/lib/isulad/engines/lcr目录下加载oci配置文件config.json
    ...
    // 在树中根据id找到storage_rootfs，进行rootfs的挂载工作
