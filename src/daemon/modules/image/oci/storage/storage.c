...
static int make_storage_directory(struct storage_module_init_options *opts)
{
    ...
    if (set_file_owner_for_user_remap(opts->storage_root, conf_get_isulad_user_remap()) != 0) {
...
/*
struct storage_rootfs_options * make_storage_rootfs_opts()
{
    struct storage_rootfs_options * ret = util_common_calloc_s(sizeof(struct storage_rootfs_options));
    ret->id_mapping_opts.host_uid_mapping = true;
    ret->id_mapping_opts.host_gid_mapping = true;
    ret->id_mapping_opts.uid_map = util_common_calloc_s(sizeof(struct id_map));
    ret->id_mapping_opts.gid_map = util_common_calloc_s(sizeof(struct id_map));
    ret->id_mapping_opts.uid_map->container_id = 0;
    ret->id_mapping_opts.uid_map->host_id = 165536;
    ret->id_mapping_opts.uid_map->size = 65535;
    ret->id_mapping_opts.gid_map->container_id = 0;
    ret->id_mapping_opts.gid_map->host_id = 165536;
    ret->id_mapping_opts.gid_map->size = 65535;
    ret->id_mapping_opts.uid_map_len = 1;
    ret->id_mapping_opts.gid_map_len = 1;
    return ret;
}
*/