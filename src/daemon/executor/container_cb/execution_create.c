                                             ...
                                             container_config_v2_common_config *v2_spec)
{
    ...
    // container_spec?
    // 从/var/lib/isulad/storage/overlay/container_id/merged合并配置信息
...
static int create_container_root_dir(const char *id, const char *runtime_root)
{
    ...
    if (set_file_owner_for_user_remap(container_root, conf_get_isulad_user_remap()) != 0) {
                                                 ...
                                                 json_map_string_string *storage_opt, char **real_rootfs)
{
    ...
    /*
    if(conf_get_isulad_user_remap() != NULL){
        request->rootfs_opts = make_storage_rootfs_opts();
    }*/
    // 准备rootfs的info，storage_rootfs
...
static int create_v2_config_json(const char *id, const char *runtime_root, container_config_v2_common_config *v2_spec)
{
    ...
    // 保存到/var/lib/isulad/engines/lcr中
 ...
 */
// create命令的回调执行函数,create命令的主逻辑。主要获取容器配置信息并保存到本地
int container_create_cb(const container_create_request *request, container_create_response **response)
{
    ...
    // 开始创建容器中
    // 从request中获取image_type,image_name
    ...
    // 从request中获取容器基础配置信息（host_spec,container_spec）
    ...
    // 将container_spec和image_config添加到v2_spec
    v2_spec_fill_basic_info(id, name, image_name, image_type, container_spec, v2_spec);
    // 在/var/lib/isulad/engines/lcr中保存容器配置参数(host_spec,container_spec,image_config)
    ...
    // 为sys_container设置id重映射（可在oci_spec中设置idmap）
    ...
    // host_channel普通容器默认没有
    host_channel = dup_host_channel(host_spec->host_channel);
    // 根据user_remap更改host_channnel文件的权限（容器与主机通信的共享内存）
    ...
    // 将oci_spec写入/var/lib/isulad/engines/lcr中
