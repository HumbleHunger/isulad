...
char *conf_get_isulad_user_remap()
{
    struct service_arguments *conf = NULL;
    char *user_remap = NULL;
    ...
    user_remap = conf->json_confs->userns_remap;
...
/*int set_file_owner_for_user_remap(const char *filename)
{
    char *user_remap = conf_get_isulad_user_remap();
    if (user_remap != NULL) {
