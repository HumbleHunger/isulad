...
static int update_graph_for_user_remap(struct service_arguments *args)
{
    int ret = 0;
    ...
    if (args->json_confs->user_remap == NULL){
        goto out;
    }

    if (util_parse_user_remap(args->json_confs->user_remap, &host_uid, &host_gid, &size)) {
        ERROR("Failed to split string '%s'.", args->json_confs->user_remap);
        ret = -1;
        goto out;
    }
...
static int update_server_args(struct service_arguments *args)
{
    ...
    if(update_graph_for_user_remap(args) != 0){
        ret = -1;
        goto out;
    }

    if (update_tls_options(args) != 0) {
        ret = -1;
        goto out;
    }
                                  ...
                                  const char *fifo_full_path)
{
    ...
    // 考虑修改根目录权限
    if (set_file_owner_for_user_remap(args->json_confs->graph, conf_get_isulad_user_remap()) != 0) {
